...
import type { KeyPress, ValidationResult } from "../core";
import type { InteractiveCLI } from "./cli";
...
// Core page interface
export interface Page<TState = unknown, TAction = unknown> {
	// Basic properties
	id: string;
	title: string;
	description?: string;
...
export class PageBuilder<TState = unknown, TAction = unknown> {
	private page: Partial<Page<TState, TAction>> = {};

	static create<TState = unknown, TAction = unknown>(
		id: string,
		title: string,
	): PageBuilder<TState, TAction> {
	...
	return PageBuilder.create<TState>(id, title)
		...
		.render(async (cli, state) => {
			...
			if (validator) {
				...
				state.input.isValid = validation.isValid;
				state.input.error = validation.error;
			} else {
				state.input.isValid = true;
				state.input.error = undefined;
			}
		})
		.handleKey((key, state) => {
...
export function createSelectPage<TState extends { select: SelectPageState }>(
	...
	return PageBuilder.create<TState>(id, title)
		...
		.render(async (cli, state) => {
			...
			// Update state
			state.select.selectedValues = result;
			state.select.selectedIndex = options.indexOf(result[0]);
			state.select.isMultiple = multiple;
		})
		.handleKey((key, state) => {
...
export function createConfirmPage<TState extends { confirm: ConfirmPageState }>(
	...
	return PageBuilder.create<TState>(id, title)
		...
		.render(async (cli, state) => {
			...
			// Update state
			state.confirm.confirmed = result;
		})
		.handleKey((key, state) => {
