...
import type { EventHandler, FrameworkOptions, StateListener } from "../core";
 ...
 */
export class InterShellFramework<TState, TAction extends { type: string; payload?: unknown }> {
	...
	// Event system
	on<K extends keyof FrameworkEvents<TState>>(event: K, handler: EventHandler): void {
	...
	off<K extends keyof FrameworkEvents<TState>>(event: K, handler: EventHandler): void {
	...
	emit<K extends keyof FrameworkEvents<TState>>(event: K, data: FrameworkEvents<TState>[K]): void {
		const handlers = this.eventHandlers.get(event);
		if (handlers) {
	...
	private async handlePageAction(
		action: PageAction,
		currentPage: Page<TState, TAction>,
	): Promise<boolean> {
		switch (action.type) {
			case "NEXT_PAGE": {
			...
			case "CUSTOM":
				...
				if (action.payload) {
					this.dispatch(action as TAction);
				}
				break;
