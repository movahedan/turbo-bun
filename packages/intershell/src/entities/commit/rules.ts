...
class CommitRules {
	...
	private rules: Record<string, CommitRule[]> = {};
	...
	getRules(): Record<string, CommitRule[]> {
		return this.rules;
	}
	...
	private createRules(): Record<string, CommitRule[]> {
		const config = this.config;
		return {
			mergeCommit: [
				{
					section: "subject",
					question: "🔀 What's the merge commit message?",
					validator: (commit): true | string => {
						return commit.message.isMerge ? true : "is not a merge commit";
					},
				},
			],
			conventional: [
