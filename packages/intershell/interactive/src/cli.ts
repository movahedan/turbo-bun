...
import { colorify, cliTools } from '@intershell/core';
...
export class InteractiveCLI implements IInteractiveCLI {
  ...
  private currentPromise: ControlledPromise<any> | null = null;
  ...
  async select(question: string, options: string[], config: SelectConfig = {}): Promise<string[]> {
    ...
    this.currentPromise = controlledPromise;
    ...
    const render = () => {
      ...
      if (quickActions.length > 0) {
        cliTools.renderQuickActions(quickActions);
      }

      this.console.log();
      this.console.log(colorify.gray('Use arrow keys to navigate, Enter to select' + (multiple ? ', Space to toggle' : '')));
    };
    ...
    const keyHandler = (key: KeyPress) => {
      if (key.name === 'up' && selectedIndex > 0) {
        ...
        if (multiple) {
          ...
          const result = Array.from(selectedIndices).sort().map(i => options[i]);
